...
import Swal from "sweetalert2";
import { fireDynamicSwal } from "../utils/dynamicSwal.jsx";
import { createSwalField, createSwalTextarea, createSwalDateField } from "../utils/swalField.jsx";
import { gebi } from "../utils/getElementById.jsx";
import { StaticDropdownList } from "../utils/DropdownList.jsx";
import { AREAS_PERMITIDAS_EN_MAYUSCULA } from "../../constants/ElectivoConstants.jsx";
...
async function editElectivoInfo(electivo) {
  const { value } = await Swal.fire({
    ...
    html: `
      ${createSwalField(1, "Nombre", electivo.nombre)}
      ${createSwalTextarea(2, "Descripción", electivo.descripcion)}
      ${createSwalField(3, "Cupos", electivo.cupos)}
      ${createSwalField(4, "Créditos Requeridos", electivo.creditos_requeridos)}
      ${createSwalDateField(5, "Apertura", electivo.apertura)}
      ${createSwalDateField(6, "Cierre", electivo.cierre)}
      ${StaticDropdownList(AREAS_PERMITIDAS_EN_MAYUSCULA, electivo.area, "swal2-input7", "m-1", false)}
      ${createSwalField(8, "Semestre Mínimo", electivo.semestre_minimo)}
      ${StaticDropdownList(CARRERAS_PERMITIDAS, electivo.carreras, "swal2-input9", "m-1", false)}
    `,
    showCancelButton: true,
    confirmButtonText: "Editar",
    theme: "dark",
...
export const useEditElectivo = (fetchElectivos) => {
  const handleEditElectivo = async (id, electivo, isAdmin) => {
